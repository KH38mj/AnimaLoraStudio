# 0010 — LoRA 评估指标体系与可审查 PR 拆分

**状态**：Proposed
**日期**：2026-05-27
**决策者**：@WalkingMeatAxolotl

> **维护约定**：本 ADR 只定义 LoRA 训练后评估体系的目标、边界和 PR 拆分。
> 运行时代码、模型依赖、UI 和具体指标实现都在后续 PR 分别落地。
...
| PR | 范围 | 明确不做 | 阻塞关系 |
|---|---|---|---|
| 0 | 本 ADR + 文档索引 | 不写运行时代码 | 无 |
| 1 | Eval manifest：固定 held-out images / captions / prompts / seeds / metadata | 不生成图、不算指标 | 0 |
| 2 | Eval sample runner：按 manifest 对 checkpoint 出图，保存 grid / 单图 / metadata | 不接 CLIP / DINO / CMMD | 1 |
