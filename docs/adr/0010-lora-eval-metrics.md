...
| 5（当前 stacked PR） | DINO-I：新增 `eval_dino` job，读取 generated/reference image pairs，写入 `metrics.json` 的 `dino_i` | 不改 CLIP 逻辑、不做 diversity / copy-risk / paired CMMD²、不做诊断 UI | 3, 4 |
